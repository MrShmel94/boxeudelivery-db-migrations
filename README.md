...
`V22__add_user_daily_purchase_rates.sql` adds immutable account-scoped daily purchase-rate snapshots with one active value per date and directed pair, links every new item-rate snapshot to the exact reused daily revision, and preserves pre-V22 item rows through a nullable historical link. `V23__correct_user_daily_purchase_rate_legacy_links.sql` corrects the legacy actor-subject match without modifying the already-applied V22: only unambiguous equal legacy values are linked, while a conflicting current daily value remains authoritative and untouched. Daily correction appends a reasoned successor and leaves previously linked item snapshots unchanged.

`V60__create_owner_scoped_customer_fx_quotes.sql` adds immutable customer FX quote snapshots linked to the exact current purchase-cost and customer-price revisions. A quote stores the observed market rate and confidential customer rate for one explicit directed pair, belongs either to one persistent account or one project supplier group, and permits only one active snapshot per owner and price-revision pair. Corrections append a reasoned successor; generic audit data receives only the new aggregate type and never stores either rate.

`V63__bind_customer_fx_quotes_to_purchase_date.sql` removes the client-controlled quote date from the persisted meaning. Existing snapshots are aligned with the exact purchase-cost revision and the column is renamed to `purchase_date`; the migration stops instead of relabelling an unknown date when the exact purchase revision has no purchase date. Customer, company, and supplier settlement dates remain separate future facts.

`V64__create_customer_profiles.sql` separates reusable supplier-scoped customer identity from CRM authentication. It creates customer profiles and typed HTTPS contact links, backfills linked profiles for existing orders, adds immutable customer contact snapshots to orders, and makes the order account reference optional so an unregistered customer receives no CRM access or personal notifications.
